...
void main() {
    ...
    vec4 normWorldHom = uModel * vec4(vNormal, 0.0);
    vec3 normWorldCart = vec3(normWorldHom / normWorldHom.w);
    float lambertian = max(dot(lightDir, normalize(normWorldCart)), 0.0);
